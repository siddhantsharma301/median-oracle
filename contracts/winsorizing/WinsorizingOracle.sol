...
library WinsorizingOracle {
    ...
    function getSurroundingObservations(
        ...
        if (lte(time, beforeOrAt.blockTimestamp, target)) {
            if (beforeOrAt.blockTimestamp == target) {
                ...
                return (beforeOrAt, transform(beforeOrAt, target, tick, liquidity, false));
            }
        }
    ...
    function observeSingle(
        ...
        if (secondsAgo == 0) {
            Observation memory last = self[index];
            if (last.blockTimestamp != time) last = transform(last, time, tick, liquidity, false);
